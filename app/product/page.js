"use client";
import { AiTwotoneHeart, AiFillStar , AiOutlineShoppingCart, AiOutlineSearch, AiOutlineBell, AiOutlineHeart } from "react-icons/ai";
...
const Product = () => {
  ...
  let product_id = router.get('product_id')
  ...
  const addToCart = (id)=>{
    axios.post(`${BASE_URL}/api/customer/cart/add/${id}/`,
    {},
    {headers:{"X-CSRFTOKEN": "XY0p5KSoVXwePQWNIZSWps99AIyhspW3AfDkPXg8a5QqzxXIi4jt5cgZqDrUlFh3"}}
    )
    .then((data)=>{
        if(data.status === 200){
          toast.success(`${productData && 'name' in productData ? productData.name : "Current Product"} has been added to Cart`,{
    ...
    <div className="relative bg-white w-full  overflow-hidden text-left text-xl text-gray font-inter flex px-11 flex-col">
        ...
        <div className="grid col-span-full lg:col-span-5 grid-cols-3 grid-rows-5 gap-1 h-full">
          <img
            ...
            alt=""
            src={`${productData ? productData.image : false} || "/auth_images/signup2.png" }`}
          />
          <img
          ...
          <div className=" font-light text-darkgray flex items-center ">
            {`${productData.description ? ( details ? productData.description : productData.description.slice(0,350) ): `Experience a radiant transformation with our Radiant Glow Serum.
            ...
            <div className=" text-blue-600 cursor-pointer flex items-center w-[131px] h-5" onClick={showDetails}>
          ...
          <IconContext.Provider onMouseEnter={(e)=>e.target.fill = "black"} value={{className:"hover:bg-red", fill: isHeartFilled ? 'red' : 'white'}}>
          <div onClick={handleHeartClick}>
            {isHeartFilled ? <AiTwotoneHeart className="w-10 " fill="red" onMouseOver={(e)=>e.target.fill = "black"}  /> : <AiOutlineHeart className="w-10 bg-white fill-red hover:bg-grey" />}
          </div>
        </IconContext.Provider>
          </div>
            Like this product?
          </div>
          <Link href={`/checkout?type=product&id=${product_id}`}>
          ...
          <button onClick={()=>addToCart(product_id)} className=" lg:grid-col-span-2 lg:row-start-2 row-start-3 text-center bg-whitesmoke flex items-center justify-center w-[276px] h-[75px]">
            <AiOutlineShoppingCart className="w-10" mx-4></AiOutlineShoppingCart>
            Add To Cart
          </button>
          </div>
        ...
        <div className="flex flex-col gap-8 justify-evenly my-4 lg:my-0 lg:w-1/3  ">
          ...
          <div className=" text-darkgray flex items-center ">
            <div className="[line-break:word] w-full h-full">
              ...
              <a className="m-0 text-blue-600" onClick={showDescription}>{description ? "Show Less" : "More..."}</a>
            </div>
          </div>
          </div>
          <div className="flex flex-col gap-8 justify-evenly my-4 lg:my-0 lg:w-1/3">
          ...
          <div className="text-inherit font-inherit text-darkgray whitespace-pre-wrap flex items-center">
            <span className="[line-break:anywhere] w-full">
              <p className="m-1">
                We strive to provide a seamless and efficient delivery experience
                for our customers who choose to purchase our Radiant Glow Serum.
                Here are the details of our delivery process:
              </p>
          ...
          />
          <AiOutlineHeart
          className="absolute bottom-[20px] right-[20px] w-[22px] h-[22px]"
          ></AiOutlineHeart>
          </div>
          <div className="w-80 h-80 relative block object-cover flex">
          <img
          ...
          Reviews
            ...
            <ul className="pl-[27px] w-screen mx-8" style={{listStyle: "square"}}>
            <li className="mb-0">
            "This serum is a game-changer! My skin feels incredibly hydrated
            and looks so radiant. It has improved my complexion and reduced
            the appearance of fine lines." - Emily S.
            </li>
            <li className="mb-0">
            "I've tried many serums, but this one truly delivers. My skin
            feels smoother, more plump, and has a healthy glow. It's a staple
            in my skincare routine now." - James P.
            </li>
            <li>
            "The Radiant Glow Serum has transformed my skin. I struggled with
            dullness and uneven texture, but after using this serum, my
            complexion has never looked better. Highly recommended!" - Sarah
            L.
            </li>
            </ul>
            </div>
            <div className=" inline-block w-[169px] h-7">
            More like this
            </div>
            </div>
            </div>
